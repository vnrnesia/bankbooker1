...
const Navbar = () => {
  ...
  // Mobil ürünler dropdown aç/kapa
  const toggleMobileProducts = () => {
    setMobileProductsOpen((v) => !v);
  };

  // Mobil menü açıkken scroll engellemek için
  useEffect(() => {
    if (mobileMenuOpen) {
  ...
  return (
    <>
      ...
      <nav
        className={`fixed w-full bg-white/70 backdrop-blur-xs shadow-sm z-50 transition-all duration-300 ease-in-out ${
      ...
      >
        <div className="mx-auto max-w-10/12 flex items-center justify-between p-4">
          ...
          <button
            ...
            className="md:hidden flex flex-col justify-center items-center w-10 h-10 space-y-1 focus:outline-none z-50"
            onClick={toggleMobileMenu}
          >
            <span
              className={`block h-0.5 w-6 bg-black rounded transform transition duration-300 ease-in-out origin-left ${
                mobileMenuOpen ? "rotate-45 translate-y-1.5" : ""
              }`}
            ></span>
            <span
              className={`block h-0.5 w-6 bg-black rounded transition duration-300 ease-in-out ${
        ...
        {/* Mobil Menü Yukarıdan Kayarak Açılan (içeriğe göre yükseklik) */}
        <div
          className={`absolute left-0 w-full bg-white shadow-md z-40 origin-top transition-transform duration-300 ease-in-out md:hidden ${
